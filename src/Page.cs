...
public partial class Database : DbContext
{
    private static readonly Regex rxColumns =
            new(@"\A\s*SELECT\s+((?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|.)*?)(?<!,\s+)\bFROM\b",
            RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
    private static readonly Regex rxOrderBy = new (
        @"\bORDER\s+BY\s+(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?(?:\s*,\s*(?:\((?>\((?<depth>)|\)(?<-depth>)|.?)*(?(depth)(?!))\)|[\w\(\)\.])+(?:\s+(?:ASC|DESC))?)*",
        RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);
    private static readonly Regex rxDistinct = new (@"\ADISTINCT\s",
        RegexOptions.IgnoreCase | RegexOptions.Multiline | RegexOptions.Singleline | RegexOptions.Compiled);

    private async Task<PageParser<T>> PageParseAsync<T>(string sql, int page, object param, int itemsPerPage)
    {
        ...
        var total = await QueryFirstOrDefaultAsync<long>(sqlCount, param).ConfigureAwait(false);
    ...
    public async Task<Page<T>> PageAsync<T>(string sql, int page = 1, object param = null, int itemsPerPage = 10)
    {
        var p = await PageParseAsync<T>(sql, page, param, itemsPerPage).ConfigureAwait(false);
    ...
    public async Task<Page<TReturn>> PageAsync<TFirst, TSecond, TReturn> (
        string sql, Func<TFirst, TSecond, TReturn> map, int page = 1, object param = null, int itemsPerPage = 10, string splitOn = "Id")
    {
        var p = await PageParseAsync<TReturn>(sql, page, param, itemsPerPage).ConfigureAwait(false);
